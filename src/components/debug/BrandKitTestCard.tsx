...
function ColorSwatch({ name, hex }: { name: string; hex: string | null }) {
  ...
  return (
    ...
    >
      <div
        className="w-16 h-16 rounded-lg border border-gray-200 shadow-sm group-hover:scale-105 transition-transform"
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block bg-gray-100 text-gray-800 text-xs rounded-full px-3 py-1 mr-2 mb-2 border border-gray-200">
      {children}
    </span>
  )
}
...
export default function BrandKitTestCard() {
  ...
  async function run() {
    ...
    try {
      // Stage indicator is visual only; the function does both steps server-side.
      setTimeout(() => {
        if (status === 'loading') setStage('Inferring brand voice...')
      }, 4000)

      const { data, error: fnErr } = await supabase.functions.invoke('test-brand-kit', {
  ...
  return (
    <section className="bg-white border-2 border-indigo-200 rounded-lg p-5 space-y-4">
      <div className="flex items-start gap-3">
        <span className="text-xs font-mono bg-indigo-100 text-indigo-800 rounded px-2 py-1">
          BRAND KIT
        </span>
        <div className="flex-1">
          <h2 className="font-semibold text-gray-900">Brand Kit Extraction (Test)</h2>
          <p className="text-sm text-gray-600 mt-1">
            Enter any brand website URL. Scrapes the site (Firecrawl), extracts colors / fonts / logo,
            and infers brand voice via AI. Sandbox only — does not write to the database.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-5 pt-2">
          ...
          <div className="flex items-center gap-4 p-4 rounded-lg border border-gray-200 bg-gray-50">
            {kit.logo_url ? (
              <img
                src={kit.logo_url}
                alt="logo"
                className="w-16 h-16 object-contain rounded bg-white border border-gray-200"
              ...
              <img
                src={kit.favicon_url}
                alt="favicon"
                className="w-16 h-16 object-contain rounded bg-white border border-gray-200 p-2"
              />
            ) : (
            ...
            <div className="flex-1">
              <div className="text-xl font-bold text-gray-900">
                {kit.brand_name ?? 'Unknown brand'}
              </div>
              {kit.tagline && (
                <div className="text-sm text-gray-600 italic">"{kit.tagline}"</div>
              )}
              <a
                href={kit.website_url}
                target="_blank"
                rel="noreferrer"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="border border-gray-200 rounded p-3">
                ...
                <div
                  ...
                  style={{ fontFamily: kit.fonts.heading ?? undefined }}
                >
                  {kit.fonts.heading ?? '—'}
                </div>
              </div>
              <div className="border border-gray-200 rounded p-3">
                ...
                <div
                  ...
                  style={{ fontFamily: kit.fonts.primary ?? undefined }}
                >
                  {kit.fonts.primary ?? '—'} — The quick brown fox jumps.
                </div>
              </div>
            </div>
            {kit.fonts.all.length > 0 && (
              <div className="mt-2 text-xs text-gray-500">
                Detected: {kit.fonts.all.join(', ')}
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-gray-200 rounded p-3">
              ...
              <div>
                {kit.tone_of_voice
                  ? kit.tone_of_voice.split(',').map((t, i) => <Chip key={i}>{t.trim()}</Chip>)
          ...
          <div className="border border-gray-200 rounded p-3">
            ...
            <div>
              {kit.product_categories.length
                ? kit.product_categories.map((c, i) => <Chip key={i}>{c}</Chip>)
                : <span className="text-sm text-gray-400">—</span>}
            </div>
          </div>
          ...
          <div className="border border-gray-200 rounded p-3">
            ...
            {kit.value_propositions.length ? (
              <ul className="list-disc ml-5 text-sm text-gray-700 space-y-1">
                {kit.value_propositions.map((v, i) => (
                  <li key={i}>{v}</li>
                ))}
              </ul>
            ) : (
          ...
          {kit.screenshot_url && (
            <details
              open={showShot}
              onToggle={(e) => setShowShot((e.target as HTMLDetailsElement).open)}
          ...
          >
            ...
            <div className="relative">
              <button
                onClick={copyRaw}
                className="absolute top-2 right-2 bg-gray-700 hover:bg-gray-600 text-gray-100 text-xs rounded px-2 py-1 z-10"
              >
                {copied ? 'Copied!' : 'Copy'}
              </button>
