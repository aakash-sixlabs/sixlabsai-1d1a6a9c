...
export const DataReviewStep = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      // Pull from new normalized schema and join in JS
      const [campaignsRes, adSetsRes, adsRes, creativesRes, perfRes] = await Promise.all([
        supabase.from("campaigns").select("id, name, status"),
        supabase.from("ad_sets").select("id, name, campaign_id"),
        supabase.from("ads").select("id, name, status, ad_set_id"),
        supabase
          .from("ad_creatives")
          .select("ad_id, headline, primary_text, stored_image_url, image_url, cta_type"),
        supabase
          .from("ad_performance_daily")
          .select("ad_id, spend, impressions, clicks, ctr, cpc, cpm, roas"),
      ]);
      ...
      perf.forEach((p: any) => {
        const agg = perfByAd.get(p.ad_id) || {
          spend: 0, impressions: 0, clicks: 0,
          ctrSum: 0, cpcSum: 0, cpmSum: 0, roasSum: 0, n: 0,
        };
        agg.spend += Number(p.spend || 0);
        agg.impressions += Number(p.impressions || 0);
        agg.clicks += Number(p.clicks || 0);
        agg.ctrSum += Number(p.ctr || 0);
        agg.cpcSum += Number(p.cpc || 0);
        agg.cpmSum += Number(p.cpm || 0);
        agg.roasSum += Number(p.roas || 0);
        agg.n += 1;
        perfByAd.set(p.ad_id, agg);
      });

      const built: ReviewRow[] = ads.map((ad: any) => {
        const adSet = adSetMap.get(ad.ad_set_id);
        const campaign = adSet ? campaignMap.get((adSet as any).campaign_id) : undefined;
        const creative = creativeByAd.get(ad.id);
        const agg = perfByAd.get(ad.id);
        return {
          id: ad.id,
          campaign_name: (campaign as any)?.name || null,
          campaign_status: (campaign as any)?.status || null,
          adset_name: (adSet as any)?.name || null,
          ad_name: ad.name,
          ad_status: ad.status,
          creative_title: creative?.headline || null,
          creative_body: creative?.primary_text || null,
          creative_image_url: creative?.stored_image_url || creative?.image_url || null,
          call_to_action: creative?.cta_type || null,
          spend: agg?.spend ?? null,
          impressions: agg?.impressions ?? null,
          clicks: agg?.clicks ?? null,
          ctr: agg?.n ? agg.ctrSum / agg.n : null,
          cpc: agg?.n ? agg.cpcSum / agg.n : null,
          cpm: agg?.n ? agg.cpmSum / agg.n : null,
          roas: agg?.n ? agg.roasSum / agg.n : null,
        };
      });
  ...
  const uniqueAds = useMemo(() => rows, [rows]);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">{uniqueAds.length} ads · {dailyCount} daily rows</p>
          </div>
