  ...
  const rows: FlatRow[] = useMemo(() => {
    return ads.map(ad => {
      ...
      return {
        ...
        adSet,
        ad,
        creative: creativeByAd.get(ad.id),
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {campaigns.length} campaigns · {adSets.length} ad sets · {ads.length} ads · {creatives.length} creatives
            </p>
          </div>
                  ...
                  <tr key={row.ad.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors align-top">
                    ...
                    <td className="px-3 py-3">
                      {imgs.length > 0 ? (
                        ...
                        <div className="w-20 h-20 rounded-md bg-muted/30 flex items-center justify-center text-muted-foreground text-xs">
                          No img
                        </div>
                      )}
                    </td>
                    {/* Campaign */}
                    <td className="px-3 py-3">
                      <div className="font-medium text-foreground max-w-[160px] truncate" title={row.campaign.campaign_name}>
                        {row.campaign.campaign_name}
                      </div>
                      <div className="text-xs text-muted-foreground">{row.campaign.objective || ""}</div>
                    </td>
                    {/* Ad Set */}
                    <td className="px-3 py-3">
                      <div className="text-foreground max-w-[160px] truncate" title={row.adSet.adset_name}>
                        {row.adSet.adset_name}
                      </div>
                    </td>
                    {/* Ad Name */}
                    <td className="px-3 py-3">
                      <div className="text-foreground max-w-[180px] truncate" title={row.ad.ad_name}>
                        {row.ad.ad_name}
                      </div>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                        row.ad.status === "ACTIVE" ? "bg-green-500/10 text-green-600" : "bg-muted text-muted-foreground"
                      }`}>{row.ad.status || "—"}</span>
                    </td>
                    {/* Creative type */}
                    <td className="px-3 py-3">
                      {row.creative && (
                        <span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap ${
                          row.creative.creative_type.startsWith("static") ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                        }`}>
                          {row.creative.creative_type.replace("_", " ")}
                        </span>
                      )}
                    </td>
                    {/* Headline */}
                    <td className="px-3 py-3">
                      <div className="max-w-[180px]">
                        {row.creative?.headline && (
                          <div className="text-foreground text-xs line-clamp-2">{row.creative.headline}</div>
                        )}
                        {row.creative?.primary_text && (
                          <div className="text-muted-foreground text-xs line-clamp-2 mt-0.5">{row.creative.primary_text}</div>
                        )}
                      </div>
                    </td>
                    {/* Metrics */}
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={12} className="px-3 py-12 text-center text-muted-foreground">
                    No ad data found. Try syncing your account first.
                  </td>
                </tr>
              )}
            </tbody>
