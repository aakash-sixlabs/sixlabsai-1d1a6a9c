 ...
 * LandingV2Step
 *
 * Visual replica of LandingV1Step wired to the mocked onboarding flow at
 * `/loginv2`. Only Meta auth + ad account fetch are live; everything after
 * is mocked inside LoginV2.tsx.
 */
...
const ONBOARDING_V2NEW_PATH = "/loginv2";

export const LandingV2Step = () => {
  ...
  const handleTokenSubmit = async () => {
    ...
    try {
      ...
      navigate(`${ONBOARDING_V2NEW_PATH}?meta=connected`);
    } catch (err: any) {
  ...
  const handleDevBypass = () => {
    ...
    enableDevSession();
    updateState({ metaConnected: true });
    navigate(`${ONBOARDING_V2NEW_PATH}?dev=true&meta=connected&new=true`);
  };
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-[45%] flex flex-col items-center justify-center px-6 sm:px-12 lg:px-16 bg-background py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
          ...
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground/70 text-center mb-6">Onboarding · v2</p>
          ...
          >
            ...
            Dev Mode — Test New User Flow (v2)
          </Button>
